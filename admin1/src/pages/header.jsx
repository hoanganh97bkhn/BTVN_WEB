...
class header extends Component {
    ...
    handleListOffline = (item) => {
      // item.map((val, index)=>{
      //   val.time =new Date();
      //   return (this.setState({
      //     listOrder : [...this.state.listOrder , val]
      //   }))
      // })
      let sum = 0;
        item.production.forEach(element => {
           return sum += (Number)(element.price) * (Number)(element.quantity)
        });
      item.total = sum;
      item.time = new Date();
      this.setState({
        listOrder : [...this.state.listOrder, item]
      })
    }

    setDataOrder = (item) => {
      this.setState=({
        dataOrder : this.state.dataOrder.concat(item)
      })
    }
    ...
    render() {
      ...
      console.log(this.props.user, this.props.pass)
        ...
        {this.state.activeTab === "1"   ?  <OrderManagement setData={this.setDataOrder} getData={this.state.dataOrder} tempSuccess={this.state.listOrder} listSuccess = {this.listSuccess} tempLater={this.state.listLater} listLater={this.listLater}/> : null}
        ...
        {this.state.activeTab === '5.2' ?  <Other/> : null}
        {this.state.activeTab === '5.3' ?  this.props.islogout(true) :null}
        {this.state.activeTab === '6'   ?  <ListOrderOffline updateListOffline={this.handleListOffline} /> : null}
            </div>
        );
    }
}
