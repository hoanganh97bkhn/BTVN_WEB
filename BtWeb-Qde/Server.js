...
server.use(express.static('result_vote'));
...
server.post('/home', (req,res)=>{
    ...
    if(req.body.btnClick == "ketquavote"){
        let htmlSource = fs.readFileSync('./result_vote/resultVote.html',"utf-8");
        const yes = question.yes;
        const no = question.no;
        htmlSource = htmlSource.replace('{question}', question.content);
        htmlSource = htmlSource.replace('{sub}',yes+no);
        htmlSource = htmlSource.replace('{yes}', yes/(yes+no)*100 );
        htmlSource = htmlSource.replace('{no}', no/(yes+no)*100 );
    ...
    if(req.body.btnClick == "cauhoikhac"){
        index=randQuestion();
        question=arrayQuestion[index];
        let htmlSource2 = fs.readFileSync('./public_page/home-page.html',"utf-8");
        res.status(200).send(htmlSource2.replace('{question}', question.content));
    }
