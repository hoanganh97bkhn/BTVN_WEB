...
class OrderManagement extends Component {
    ...
    componentDidMount () {
        ...
        if(temp === 0) {
            ...
            axios({
                ...
                .then((response) => {
                  ...
                  this.setState({
                    ...
                    orders : this.state.orders.concat(this.props.getData).concat(response.data).map( toDoitem => 
                        {
                            return {
                                ...toDoitem,
                    ...
                    items :  this.state.orders.concat(this.props.getData).concat(response.data).map( toDoitem => 
                        {
                            return {
                                ...toDoitem,
                                search : toDoitem.name + toDoitem.address + toDoitem.production
            ...
            this.setState({
                ...
                orders : this.state.orders.concat(this.props.getData).map( toDoitem => 
                    {
                        return {
                            ...toDoitem,
                            search : toDoitem.name + toDoitem.address + toDoitem.production
                ...
                items :  this.state.orders.concat(this.props.getData).map( toDoitem => 
                    {
                        return {
                            ...toDoitem,
                            search : toDoitem.name + toDoitem.address + toDoitem.production
    ...
    successOrderForm = (index) => {
        ...
        axios({
            ...
            method: `post`,                                                                                  //upload data list-later to server
            data : this.state.orders[index]
            })
        .then( ()=>{
         this.setState({
    ...
    render() {
        ...
        if(this.state.tempSuccess === true) {
            this.props.listSuccess(this.state.listSuccess, this.state.orders);                        //dkien update list-success later handle input change
        ...
        if(this.state.tempLater === true) {
            this.props.listLater(this.state.listLater, this.state.orders );                          //dkien update list-success later handle input change
                ...
                <Table hover>
                    <thead style={{backgroundColor : '#f5f6fa'}}>
                        <tr>
                            ...
                            <th>Address</th>
                            <th>Production</th>
                            <th>Total Price (VND) <ButtonDropdown direction="up"></ButtonDropdown></th>
                        </tr>
                    </thead>
                    <tbody>
                        {_arrayobj.map((item, index) => { return (
                            <tr className='tablebody' key={index} onDoubleClick = { (e) => {this.handleClickRow(item, index)}}>
                                ...
                                <td>{item.production.map((temp,indexx)=>(
                                    <p key={indexx}> {temp.name} * ({temp.quantity}) = {temp.price} *   </p>
                                ))}</td>
